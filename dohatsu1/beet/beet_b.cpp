...
void addM(int v){
  ...
  if(lr[v]) {
    if(~match[v]){
      ans.push_back(getE(v2[v],v2[match[v]]));
      addM(match[v]);
    }
  }else{
    for(int i=0;i<G[v].size();i++){
...
void addA(){
  ...
  for(int i=0;i<V;i++){
    if(!lr[i]&&!used[i]) {
      if(~match[i]&&G[i].size()==1) ans.push_back(getE(v2[i],v2[match[i]]));
...
int main(){
  ...
  for(i=0;i<v2.size();i++) {
    cout << v2[i] << ":";
    for(j=0;j<G[i].size();j++) cout << v2[G[i][j]] << ":";
    cout << endl;
  }
  //*/
  ...
  V=v2.size();
  bipartite_matching();
  //cout << ans.size() << ":" << bipartite_matching() << endl;
  memset(used,0,sizeof(used));
  for(i=0;i<V;i++) if(!used[i]) setLR(i,0);
  //for(i=0;i<V;i++) cout << v2[i] << ":" << lr[i] << ":" << match[i] << endl;
  addA();
  sort(ans.begin(),ans.end());
  cout << ans.size() << endl;
