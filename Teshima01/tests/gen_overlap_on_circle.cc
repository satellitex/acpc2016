...
const int X_MIN[2] = {-100, -100};
const int X_MAX[2] = {+100, +100};
const int Y_MIN = -5;
const int Y_MAX = +5;
const int R_MIN = 10;
const int R_MAX = 10;
...
P rotate(P const& x, double rad){
  return P(x.real()*cos(rad)-x.imag()*sin(rad),x.real()*sin(rad)+x.imag()*cos(rad));
}
...
int main(int argc, char *argv[])
{
  int cases[2][2] = {{0, 1}, {0, 0}};
    ...
    for (int t = 0; t < 2; t++) {
        ...
        for (int i = 0; i < n; i++) {
          rx[i] = X[cases[t][0]] + R[cases[t][0]];
          ry[i] = Y[cases[t][0]];
        }
        
        for (int i = 0; i < n; i++) {
          bx[i] = X[cases[t][1]] + R[cases[t][1]];
          by[i] = Y[cases[t][1]];
        }
